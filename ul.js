...
ul = {version:0.1,author:"Yuntian Chai",company:"Unreal Light",companyfullname:"上海虚幻之光数字科技有限公司"};
ul.DEBUG = true;
...
ul.shader = function(gl,type,string,params){
    ...
    if(type==ul.SHADER_TYPE.FRAG){shader = gl.createShader(gl.FRAGMENT_SHADER);}
    else if(type==ul.SHADER_TYPE.VERT){shader = gl.createShader(gl.VERTEX_SHADER);}
    else {ul.error("shader type should be either ul.SHADER_TYPE.FRAG or ul.SHADER_TYPE.VERT");return null;}
    gl.shaderSource(shader,string);gl.compileShader(shader);
    if(gl.getShaderParameter(shader, gl.COMPILE_STATUS) === false){ul.error("Shader compiler failure!");return null;}
    var info = gl.getShaderInfoLog(shader);
    if(info!==''){ul.warn('gl.getshaderInfoLog()',info);}
    ul.log("Shader compile finished.");
    return shader;
};
// Shader Program
ul.program = function(gl,params){
    var vs = ul.shader(gl,ul.SHADER_TYPE.VERT,params.vert);var fs = ul.shader(gl,ul.SHADER_TYPE.FRAG,params.frag);
...
function getGLContext(canvas){
    var gl = null;
    if (canvas == null){
        ul.error("there is no canvas on this page"); return;}
    var names = ["webgl","experimental-webgl","webkit-3d","moz-webgl"];
    for (var i = 0; i < names.length; ++i) {
        try {gl = canvas.getContext(names[i],{antialias:false});}
        catch(e) {}
        if (gl) break;
    }
    if (gl == null){ul.error("WebGL is not available");}
    else{ ul.log("You got a WebGL context!!!");
    }
    return gl;
}
//Math
...
ul.mouseController = function(renderer){
    ...
    var _mouseDownX = 0;
    var _mouseDownY = 0;
    var _lon = 0, _mouseDownLon = 0;
    var _lat = 90, _mouseDownLat = 0;
    var _lat_clamped = 0;
    var _phi = 0, _theta = 0;
